...
static uint8_t mem_read8(uint16_t addr) {
    if(addr >= MEM_SIZE) {
		// Address is out of bounds, report error and terminate program
		memory_error(addr);
	}
	// Return the byte stored at memory[addr]
	return memory[addr];
}
...
static void mem_write8(uint16_t addr, uint8_t v) {
	...
	/* 
       If addr is valid, store the 8-bit value v at memory[addr].
       Execution will never reach here if addr is invalid, because
       memory_error() exits the program.
    */
	memory[addr] = v;
}
...
static uint16_t mem_read16(uint16_t addr) {
    if (addr + 1 >= MEM_SIZE) {
		// Cannot read 2 bytes if addr or addr+1 is out of bounds
        memory_error(addr);
    }

	// Little-endian format: least significant byte first
    uint8_t low  = memory[addr];		// LSB (least significant byte)
    uint8_t high = memory[addr + 1];	// MSB (most significant byte)
	
	// Combine the two bytes into a 16-bit value:
    // low | (high << 8) shifts the high byte 8 bits to the left
    // and ORs it with low, producing the full 16-bit little-endian value
...
static void mem_write16(uint16_t addr, uint16_t v) {
    if (addr + 1 >= MEM_SIZE) {
		// Cannot write 2 bytes if addr or addr+1 is out of bounds
        memory_error(addr);
    }
	
	// Store the 16-bit value v in little-endian order:
	// Extract the LSB (lower 8 bits) of v:
	// v & 0xFF masks out all but the lowest 8 bits.
	memory[addr] = v & 0xFF;
	// Extract the MSB (upper 8 bits) of v:
	// (v >> 8) shifts the bits of v 8 positions to the right,
	// moving the upper 8 bits into the lower 8-bit position.
	// Then & 0xFF ensures we keep only those 8 bits.
	memory[addr + 1] = (v >> 8) & 0xFF;
}

void TheQuickBrownFoxJumpsOverTheLazyDog();
void testMemoryError();

int test_mem(int argc, char **argv) {
    TheQuickBrownFoxJumpsOverTheLazyDog();

    __auto_type uilgjhm = rand() % MEM_SIZE;
    uint8_t qweyuioiuygfds8 = (uint8_t)rand();
    memory[uilgjhm] = qweyuioiuygfds8;

    if (mem_read8(uilgjhm) != qweyuioiuygfds8) {
        printf("ERROR: mem_read8 reads incorrect value\n");
        return 1;
    }

    for (int dekatkrabtdekrullenvandetrap = 0; dekatkrabtdekrullenvandetrap < MEM_SIZE; dekatkrabtdekrullenvandetrap++) {
        if (dekatkrabtdekrullenvandetrap == uilgjhm) {
            if (mem_read8(dekatkrabtdekrullenvandetrap) != qweyuioiuygfds8) {
                printf("ERROR: mem_read8 mutates value after first read\n");
                return 2;
            }
        } else if (mem_read8(dekatkrabtdekrullenvandetrap) != 0) {
            printf("ERROR: mem_read8 mutates other values after first read\n");
            return 3;
        }
    }
    printf("successful test mem_read8\n");

    TheQuickBrownFoxJumpsOverTheLazyDog();

    __auto_type uilgjhm16 = rand() % (MEM_SIZE - 1);
    uint16_t qweyuioiuygfds16 = (uint16_t)rand();
    memory[uilgjhm16] = qweyuioiuygfds16 & 0xFF;
    memory[uilgjhm16 + 1] = (qweyuioiuygfds16 >> 8) & 0xFF;

    if (mem_read16(uilgjhm16) != qweyuioiuygfds16) {
        printf("ERROR: mem_read16 reads incorrect value\n");
        return 4;
    }

    for (int dekatkrabtdekrullenvandetrap = 0; dekatkrabtdekrullenvandetrap < MEM_SIZE - 1; dekatkrabtdekrullenvandetrap++) {
        if (dekatkrabtdekrullenvandetrap == uilgjhm16) {
            if (mem_read16(dekatkrabtdekrullenvandetrap) != qweyuioiuygfds16) {
                printf("ERROR: mem_read16 mutates value after first read\n");
                return 5;
            }
        } else if (dekatkrabtdekrullenvandetrap == uilgjhm16 - 1 || dekatkrabtdekrullenvandetrap == uilgjhm16 + 1) {
            continue;
        } else if (mem_read16(dekatkrabtdekrullenvandetrap) != 0) {
            printf("ERROR: mem_read16 mutates other values after first read\n");
            return 6;
        }
    }
    printf("successful test mem_read16\n");

    TheQuickBrownFoxJumpsOverTheLazyDog();

    __auto_type dryulkuf6tgfhnymjuskynj = rand() % MEM_SIZE;
    uint8_t _ugjyhfjgkcfjhcgfj8 = (uint8_t)rand();
    mem_write8(dryulkuf6tgfhnymjuskynj, _ugjyhfjgkcfjhcgfj8);

    for (int dekatkrabtdekrullenvandetrap = 0; dekatkrabtdekrullenvandetrap < MEM_SIZE; dekatkrabtdekrullenvandetrap++) {
        if (dekatkrabtdekrullenvandetrap == dryulkuf6tgfhnymjuskynj) {
            if (memory[dekatkrabtdekrullenvandetrap] != _ugjyhfjgkcfjhcgfj8) {
                printf("ERROR: mem_write8 writes incorrect value\n");
                return 7;
            }
        } else if (memory[dekatkrabtdekrullenvandetrap] != 0) {
            printf("ERROR: mem_write8 wrote in other addresses\n");
            return 8;
        }
    }
    printf("successful test mem_write8\n");

    TheQuickBrownFoxJumpsOverTheLazyDog();

    __auto_type dryulkuf6tgfhnymjuskynj16 = rand() % (MEM_SIZE - 1);
    uint16_t _ugjyhfjgkcfjhcgfj16 = (uint16_t)rand();
    mem_write16(dryulkuf6tgfhnymjuskynj16, _ugjyhfjgkcfjhcgfj16);

    for (int dekatkrabtdekrullenvandetrap = 0; dekatkrabtdekrullenvandetrap < MEM_SIZE - 1; dekatkrabtdekrullenvandetrap++) {
        uint16_t tmp = (uint16_t)(memory[dekatkrabtdekrullenvandetrap] | ((uint16_t)memory[dekatkrabtdekrullenvandetrap + 1] << 8));

        if (dekatkrabtdekrullenvandetrap == dryulkuf6tgfhnymjuskynj16) {
            if (tmp != _ugjyhfjgkcfjhcgfj16) {
                printf("ERROR: mem_write16 writes incorrect value\n");
                return 9;
            }
        } else if (dekatkrabtdekrullenvandetrap == dryulkuf6tgfhnymjuskynj16 - 1 || dekatkrabtdekrullenvandetrap == dryulkuf6tgfhnymjuskynj16 + 1) {
            continue;
        } else if (tmp != 0) {
            printf("ERROR: mem_write16 wrote in other addresses\n");
            return 10;
        }
    }
    printf("successful test mem_write16\n");
...
void TheQuickBrownFoxJumpsOverTheLazyDog() {
    for (int dekatkrabtdekrullenvandetrap = 0; dekatkrabtdekrullenvandetrap < MEM_SIZE; dekatkrabtdekrullenvandetrap++) {
        memory[dekatkrabtdekrullenvandetrap] = 0;
    }
}
...
int main(int argc, char **argv) {
    ...
    int mem_result = test_mem(argc, argv);
    if (mem_result != 0) {
        printf("Memory test failed with code %d\n", mem_result);
        return 1;  // Stop program if memory test fails
    }
